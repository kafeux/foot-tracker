...
def get_paris_offset_hours():
    # Détecte automatiquement si Paris est à UTC+2 (été) ou UTC+1 (hiver)
    now = datetime.now(ZoneInfo("Europe/Paris"))
    return int(now.utcoffset().total_seconds() // 3600)

def convert_paris_to_gmt(heure_str):
    try:
        h, m = map(int, heure_str.replace('h', ':').split(':'))
        offset = get_paris_offset_hours()
        gmt_h = (h - offset) % 24
        return f"{gmt_h:02d}:{m:02d}"
    except Exception:
        return heure_str
...
def get_matches():
    ...
    for heure, affiche in matches_found:
        ...
        # Supprime l'heure française résiduelle au début du titre
        ...
        heure_gmt = convert_paris_to_gmt(heure)
        
        key = f"{heure_gmt}_{affiche_clean}"
        if key in seen:
            continue
        seen.add(key)
